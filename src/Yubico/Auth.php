...
class Auth
{
                    ...
                    $sl = null, $timeout = null, $max_retries = null)
    {
        ...
        if (!$ret) {
            throw new AuthException('Could not parse Yubikey OTP');
        }
            ...
            while ($info = curl_multi_info_read($mh)) {
                ...
                if ($info['result'] == CURLE_OK) {
                    ...
                    if (!$wait_for_all && ($valid || $replay)) {
                        ...
                        curl_multi_close($mh);
                        if ($replay) throw new AuthException('REPLAYED_OTP');
        ...
        if ($replay) throw new AuthException('REPLAYED_OTP');
        if ($valid) return true;
        throw new AuthException('NO_VALID_ANSWER');
    }
}
